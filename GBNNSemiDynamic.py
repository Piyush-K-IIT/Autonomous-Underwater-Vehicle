...
def initialize_grid_map(rows, cols):
    ...
    # Add a dynamic obstacle of size 4x4 at a random free space
    dynamic_obstacle_size = 4
    added = False
    while not added:
        x = np.random.randint(0, rows - dynamic_obstacle_size)
        y = np.random.randint(0, cols - dynamic_obstacle_size)
        if np.all(grid_map[x:x+dynamic_obstacle_size, y:y+dynamic_obstacle_size] == 0):
            grid_map[x:x+dynamic_obstacle_size, y:y+dynamic_obstacle_size] = 1
            added = True
            print(f'Added dynamic obstacle at: ({x}, {y})')
